...
contract SIRDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SIR_Dividend_Tracker", "SIR_Dividend_Tracker") {
        claimWait = 86400;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SIR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SIR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SIR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SIR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SIR: Cannot update claimWait to same value");
...
contract SIR is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SIRDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Sir Wen Airdrop", "SIR") {
        BNBRewardsFee = 5;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 4;
        marketingWallet = payable(0xb8F89f090854CD45bfA7165904BAc38C3eB42236);
        launchpad = 0xFCb816D727635fdACd70E6De78E7F011208b9dA9;

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SIRDividendTracker();
        ...
        excludeFromFees(launchpad, true);
        excludeFromFees(address(this), true);
        
        // exclude from max tx
        _isExcludedFromMaxTx[owner()] = true;
        _isExcludedFromMaxTx[address(this)] = true;
        _isExcludedFromMaxTx[launchpad] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SIR: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SIR: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SIR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SIR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SIR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SIR: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(extraFee>0)
            {
                swapAndSendBNBToMarketing(extraFee); // swap the marketing fee on second
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToBurn = contractTokenBalance.div(totalFees.mul(2)).mul(liquidityFee);
        // swap the remaining to BNB
        uint256 toSwap = contractTokenBalance-tokensToBurn;
        ...
        uint256 bnbFee = deltaBalance.mul(liquidityFee).div(totalFees.mul(2).sub(liquidityFee));
        
        // add liquidity to uniswap
        payable(dead).transfer(tokensToBurn);
        // worthy marketing fee
        uint256 marketingAmount = deltaBalance.sub(bnbFee).div(totalFees.sub(liquidityFee)).mul(marketingFee);
        marketingWallet.transfer(marketingAmount + bnbFee);

        uint256 dividends = address(this).balance;
        (bool success,) = address(dividendTracker).call{value: dividends}("");

        if(success) {
   	 		emit SendDividends(toSwap-tokensToBurn, dividends);
        }
        
        emit SwapAndLiquify(tokensToBurn, deltaBalance);
    }
