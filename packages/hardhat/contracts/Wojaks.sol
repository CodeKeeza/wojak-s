...
contract Wojaks is ERC20, Ownable {
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    ...
    if(!swapping && to == uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromFees[from]) {
      // sell tax logic 
    }

    if(!swapping && from == uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromFees[to]) {
      // buy tax logic
    }
